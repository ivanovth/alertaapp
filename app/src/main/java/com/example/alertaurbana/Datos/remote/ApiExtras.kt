...
interface ApiExtras {
    companion object {
        ...
        val instance = Retrofit.Builder().baseUrl("https://apiextras.municusco.com/")
            .addConverterFactory(MoshiConverterFactory.create())
            .client(OkHttpClient.Builder().connectTimeout(60, TimeUnit.SECONDS) .writeTimeout(600, TimeUnit.SECONDS)
            .readTimeout(120, TimeUnit.SECONDS) .build()).build().create(ApiExtras::class.java)
    }
    ...
    suspend fun obtenerDatos(
        @Query("long") longitud: Double,
        @Query("lat") latitud: Double,
        @Query("distance") distancia: Int
    ): List<DtoDireccionesItem>
